/*
]=====> YOHANDARA M.W <=====[ ]=====> YT YOHANDARA .MW CHANNEL <=====[ ]=====> 085559240360 <=====[
*/
const {
   WAConnection,
   MessageType,
...
*/
const ownerNumber = ["6281235029264@s.whatsapp.net"]
const prem = ["62895622955538@s.whatsapp.net"]
const botName = 'AXEBOT'
const ownerName = 'YOHANDARA M.W'
const BarBarKey = 'IDxO1TFYnKADlX4pxcHa' // APIKEY DOWNLOAD FILE
const VhtearKey = 'YOHANDARAMW' // YANG DI YOUTUBE
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' I NEED FRIENDS YOHANDARA M.W '))
})

	client.on('credentials-updated', () => {
		fs.writeFileSync('./yohandara.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'ingfokan cuyy...')
	})
	fs.existsSync('./yohandara.json') && client.loadAuthInfo('./yohandara.json')
	client.on('connecting', () => {
		start('2', 'yohandara Connecting...')
	})
	client.on('open', () => {
		success('2', 'yohandara Connected')
	})
...
*/
        ...
        case 'stiker':
            ...
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                Message.sendImageAsSticker(from, imageBase64)
                .then(() => {
                    Message.reply(from, 'Here\'s your sticker')
                ...
                if (isMedia || isQuotedImage) {
                    ...
                    await Message.sendImageAsSticker(from, `data:${mimetype};base64,${result.base64img}`)
                    } catch(err) {
                    console.log(err)
	   	            await Message.reply(from, 'Maaf batas penggunaan hari ini sudah mencapai maksimal', id)
                    }
                }
            } else if (args.length === 1) {
                if (!isUrl(url)) { await Message.reply(from, 'Maaf, link yang kamu kirim tidak valid.', id) }
                Message.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ? Message.sendText(from, 'Maaf, link yang kamu kirim tidak memuat gambar.')
                    : Message.reply(from, 'Here\'s your sticker')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
                await Message.reply(from, `Tidak ada gambar! Untuk menggunakan ${prefix}sticker\n\n\nKirim gambar dengan caption\n${prefix}sticker <biasa>\n${prefix}sticker nobg <tanpa background>\n\natau Kirim pesan dengan\n${prefix}sticker <link_gambar>`, id)
            }
            break
				case 'nulis1':
				case 'tulis1':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Teksnya mana kak? Contoh : ${prefix}nulis1 YOHANDARA .MW baik hati')
				ramlan = body.slice(8)
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Teksnya mana kak? Contoh : ${prefix}nulis2 YOHANDARA .MW baik hati')
				laysha = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				...
				client.sendMessage(from, ''+tes+'\n\n_By : ⸸Yohandara M.W⸸._', text, { quoted: mek })
				await limitAdd(sender)
				break
// NINJALOGO				
				case 'ninjalogo':
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply('「❗」Contoh : ${prefix}ninjalogo YOHANDARA .MW & Gans')
				reply(ind.wait())
...
// PORNHUB
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply('「❗」Contoh : ${prefix}pornhub YOHANDARA .MW & Hub')
				reply(ind.wait())
...
//GEMBOK TEXT
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}gemboktext 11 01 2021 & YOHANDARA .MW dan Nadia')
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}glitchtext YOHANDARA .MW & Gans')
					reply(ind.wait())
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata YOHANDARA .MW`
...
*/		 
				...
				if (!isGroup) return reply(ind.groupo())
				if (!isGroupAdmins) return reply(ind.admin())				
				if (args.length < 1) return reply('Textnya mana kunyuk >_<')
					teks = body.slice(5)
...
┃⛨┊ 
┃⛨┊ུ*USER XP* : ${userXp}/${requiredXp}*
┃⛨┊ 
┃⛨┊ུ*User Level*: ${userLevel} 
┃⛨┊ 
╰▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔
`
...
*/
                ...
                addKoinUser('6281235029264@s.whatsapp.net', fee)
...
*/
            ...
            if (isLimit(sender)) return reply(ind.limitend(pusname))
              ...
              case 'covid':
                   ...
                   hasil = `Negara : ${data.result.country}\n\nKasus Aktif : ${data.result.active}\nKasus Baru : ${data.result.casesPerOneMillion}\nKasus Kematian : ${data.result.deathsPerOneMillion}\nSembuh : ${data.result.recovered}\nTest Covid-19 : ${data.result.testPerOneMillion}\nTotal Kasus : ${data.result.totalCases}\nTotal Tes Covid-19 : ${data.result.totalTest}`
                   reply(hasil)
                   break
                case 'kpop':
                  if (isBanned) return reply(ind.baned())
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner YOHANDARA .MW`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena YOHANDARA .MW baik Bot memberikan ${one}Xp >_<`)
                      }else{
...
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply('「❗」Contoh : ${prefix}cloudtext YOHANDARA .MW')
cloud = body.slice(11)
...
*/
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 AXE BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 AXE BROADCAST 」*\n\n${body.slice(4)}`)
						}
						reply('*「 SUKSES BOSKU 」*')
					}
					break
...
default : 
                  ...
                  reply(`*Kontol Bapakau Pelernya Gede Sebelah*`)
                  }
                  if (budy.includes(`Kontol`)) {
                  reply(`*Kontol Bapakau Pelernya Gede Sebelah*`)
                  }
                  if (budy.includes(`Yahaha`)) {
                  reply(`*Hayyyyuuuuukkkkk Jangan Lupa Mencuri Uang Mama Untuk Top Up*`)
                  }
                  if (budy.includes(`bacot`)) {
                  reply(`*Ampun Bang Jago Yahahahaha*`)
                  }
                  if (budy.includes(`Caranya Gimana`)) {
                  reply(`*Iya Bang? Axebot Disini Ada Yang Bisa Saya Bantu?*`)
                  }
                  ...
                  if (budy == 'cekprefix') {
                  costum(`*AXEBOT MENGGUNAKAN PREFIX [ ${prefix} ]*`, text, tescuk, cr)
                  }
                  if (budy == 'Cekprefix') {
                  costum(`*AXEBOT MENGGUNAKAN PREFIX [ ${prefix} ]*`, text, tescuk, cr)
                  }
